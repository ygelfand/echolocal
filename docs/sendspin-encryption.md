...
`track.go` is the metadata role's view of what is playing. Decoding and speaker output (`decode.go`,
`flac.go`, `output.go`) are the same code the unencrypted client used, with the output delay added.
...
for this server, `none` otherwise), the roles it offers with the player's formats and the artwork
channel it wants, the one pairing method it offers, and whether it admits unpaired access.

## Roles

The room offers four roles, the same four the unencrypted client did:

- `player@v1` plays the audio. It is the only role the room needs, and the only one it reports state
  for.
- `metadata@v1` is the track. `server/state` carries a `metadata` object whose fields are tristate:
  absent leaves a field alone, null clears it, a value sets it. `metadataState` records which keys were
  on the wire so `track.merge` can tell the three apart, and the title and artist become Home Assistant
  sensors.
- `controller@v1` is the group's transport. `server/state` carries a `controller` object listing the
  commands the server will take, and the media player's play, pause and stop buttons send one of those
  as `client/command`. Nothing the server did not list is sent.
- `artwork@v1` is the album art. One channel, 512 by 512 JPEG, kept in memory for a screen this
  device does not have. The hello's channel fields carry the names Music Assistant's library reads
  (`media_width`, `media_height`); the spec has since shortened them.

Artwork arrives as binary messages of type 8 to 11, one per channel. Music Assistant's library sends
an 8-byte server timestamp then the whole image, or the header alone to clear the channel. The spec
has since redrawn this as an announce, parts and a cancel with a flags byte; no server sends that yet
and the two cannot be told apart on the wire, so echod reads what Music Assistant produces, as it does
for audio chunks.
...
the renderer's drift correction in `output.go`, which landed separately.
