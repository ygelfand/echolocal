...
// Palette is the colours an effect draws from.
//
// An effect either inherits the one colour Home Assistant set on the ring, or comes with its own —
// and the same frame function serves both, because it asks a palette for colours rather than dimming
// a colour it was handed. That is what makes a combination worth having a line in the catalogue
// instead of another animation to write and test: a flame that runs white at its base out to red at
// its edge, four arms in four hues, sparks scattered across the wheel.
//
// A palette is sampled two ways, and which one an effect uses says something about it. Round the
// ring, where the end has to meet the beginning, or along a gradient with a hot end and a cold one.
type Palette []Color
