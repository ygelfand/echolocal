...
func TestResamplersRankByImageRejection(t *testing.T) {
	...
	image := make(map[config.Resampling]float64)
	for _, r := range []config.Resampling{config.ResampleSinc, config.ResampleLinear, config.ResampleHold} {
		u, _ := NewResampler(r)
	...
	if image[config.ResampleSinc] >= image[config.ResampleLinear] {
		t.Errorf("the filter leaves %.2f of the image, linear only %.2f",
			image[config.ResampleSinc], image[config.ResampleLinear])
	}
	if image[config.ResampleLinear] >= image[config.ResampleHold] {
		t.Errorf("linear leaves %.2f of the image, holding only %.2f",
			image[config.ResampleLinear], image[config.ResampleHold])
	}
}
