...
func startRoom(t *testing.T, unpaired bool) *room {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		s := newSession(ws, tr, newOut(nil), nil, "Test Room", r)
		r.sessions <- s
		err = s.run(ctx)
		if err != nil {
...
// room is the session's roomView: it keeps what the security sensor was told and ignores the rest.
func (r *room) setSecurity(state string) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.security = append(r.security, state)
}

func (r *room) setState(string)  {}
func (r *room) grouped(string)   {}
func (r *room) track() track     { return track{} }
func (r *room) plays(track)      {}
func (r *room) drew(int, []byte) {}
func (r *room) reason() string   { return goodbyeRestart }
...
// hangUp closes the way a server does: a close frame, then the socket. Dropping the socket alone
// leaves the room reading a reset if it happened to be writing, which is a different ending from a
// server that said goodbye and is what these tests are not about.
func (f *server) hangUp() {
