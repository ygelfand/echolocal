...
// roomView is what the session reports to: the Player, or a stand-in in tests. It is what Home
// Assistant sees of the connection, and what the group is playing through it.
type roomView interface {
	setState(string)
	setSecurity(string)
	grouped(state string)
	track() track
	plays(track)
	drew(channel int, data []byte)
	reason() string
}
...
type session struct {
	...
	// What the handshake settled and the server then declared. The reader writes these; the category is
	// read when a setting changes under a live connection, and the roles by the sync loop deciding
	// whether the room has a player to report on. stateMu covers both.
	stateMu    sync.Mutex
	...
	// else is a message the server is free to ignore, so what is not in here is not sent. Written by the
	// reader, read from Home Assistant's thread.
	cmdMu    sync.Mutex
	commands []string
...
func newSession(ws *websocket.Conn, tr *trust, o *out, bg *speaker.Arbiter, name string, ui roomView) *session {
	return &session{
...
func (s *session) run(ctx context.Context) error {
	...
	safe.Go("sendspin hangup", func() {
		select {
		case <-ctx.Done():
			_ = s.c.writeJSON(typeClientGoodbye, clientGoodbye{Reason: s.ui.reason()})
			_ = s.c.close()
		case <-s.done:
		}
	})
	...
	for {
		...
		switch {
		case msgType == msgJSON:
			if err := s.control(payload); err != nil {
				return s.closed(ctx, err)
			}
		case msgType == msgAudio:
			s.heard(payload)
		case msgType >= msgArtwork0 && msgType <= msgArtwork3:
			s.drawn(int(msgType-msgArtwork0), payload)
		default:
...
func (s *session) hello() clientHello {
	...
	return clientHello{
		...
		SupportedRoles: []string{rolePlayer, roleMetadata, roleController, roleArtwork},
...
func (s *session) control(payload []byte) error {
	...
	case typeGroupUpdate:
		...
		s.ui.grouped(g.PlaybackState)

	case typeServerState:
		var st serverState
		if err := json.Unmarshal(env.Payload, &st); err != nil {
			return fmt.Errorf("malformed server/state: %w", err)
		}
		s.said(st)
...
func (s *session) activated(act serverActivate) error {
	...
	if act.has(activityPairing) {
		s.pairingIndex++
		s.ui.setSecurity(securityPairing)
		return s.pair(act)
	}
	s.cancelPairing()
	...
	if cat == categoryLongTerm {
		s.ui.setSecurity(securityPaired)
	} else {
		s.ui.setSecurity(securityUnpaired)
	}
...
// has says whether the server has given the room a role.
func (s *session) has(role string) bool {
	s.stateMu.Lock()
	defer s.stateMu.Unlock()
...
// controller role the commands it will act on. A null clears the role's state; an absent object leaves
// it alone.
func (s *session) said(st serverState) {
	switch {
	case isNull(st.Controller):
		s.setCommands(nil)
	case len(st.Controller) > 0:
		var c controllerState
		if err := json.Unmarshal(st.Controller, &c); err == nil {
			s.setCommands(c.SupportedCommands)
			slog.Info("sendspin server commands", "commands", c.SupportedCommands)
		}
	}

	switch {
	case isNull(st.Metadata):
		s.ui.plays(track{})
	case len(st.Metadata) > 0:
		var m metadataState
		if err := json.Unmarshal(st.Metadata, &m); err == nil {
			s.ui.plays(s.ui.track().merge(&m))
		}
	}
}

func (s *session) setCommands(c []string) {
	s.cmdMu.Lock()
	s.commands = c
	s.cmdMu.Unlock()
}
...
func (s *session) tell(want ...string) bool {
	...
	for _, cmd := range want {
		...
		if err := s.c.writeJSON(typeClientCommand, clientCommand{Controller: &controllerCommand{Command: cmd}}); err != nil {
...
func (s *session) ended() {
	...
	s.out.close()
	s.bg.Gave(s.out)
	s.ui.setState(stateJoined)
}

// Binary message layouts. An audio chunk is the server-clock time its first sample is due, then the
// codec payload. An artwork message from Music Assistant's library is the same header then the image,
// and an empty image clears the channel. The spec has since redrawn artwork as an announce, parts and
// a cancel with a flags byte; no server sends that yet, and the two cannot be told apart on the wire.
const (
	audioHeader   = 8
	artworkHeader = 8
)
...
// drawn keeps an artwork message. Nothing draws it, so the timestamp goes unread.
func (s *session) drawn(channel int, body []byte) {
	if len(body) < artworkHeader {
		s.ui.drew(channel, nil)
		return
	}
	s.ui.drew(channel, body[artworkHeader:])
}
...
func (s *session) reportState() {
	if !s.has(rolePlayer) || !s.synced.Load() {
		return
	}
	c := config.Get()
	st := clientState{
