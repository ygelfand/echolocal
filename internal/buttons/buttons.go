...
// press of the action button reaches the second assistant is the satellite's business, and keeping the
// two apart is what lets the buttons work when nothing else does.
package buttons
...
type Controller struct {
	// Handle is called for every event, from the reader goroutine. It must not block: a handler that
	// waits on something is a handler that stops the next button working.
	Handle func(Event)
...
func New(handle func(Event)) *Controller { return &Controller{Handle: handle} }
...
func (c *Controller) emit(name Name, kind Kind) {
	if c.Handle != nil {
		c.Handle(Event{Name: name, Kind: kind})
	}
}
