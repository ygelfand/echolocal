...
// answerWakeWords is what the device says when Home Assistant asks what it can hear. The question
// carries the models Home Assistant hosts in its own custom_wake_words directory, each with the URL to
// fetch it from, so the answer is that set combined with what is on disk.
//
...
func (s *Satellite) answerWakeWords(offered []esphome.ExternalWakeWord) []esphome.WakeWord {
	lib := wake.Lib()
